...
function autocomplete(input, latInput, lngInput) {
    ...
    // disable form submission when user hits Enter
...
function ajaxHeart(e) {
    ...
    _axios2.default.post(this.action).then(function (result) {
        if (result.status !== 200) return;

        var isHearted = _this.heart.classList.toggle('heart__button--hearted');

        var heartNumber = (0, _bling.$)('.heart-count');
...
function loadPlaces(map) {
    var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaultLat;
    var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : defaultLng;

    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (results) {
...
function resultsHTML(stores) {
    return stores.map(function (store) {
        ...
        return _dompurify2.default.sanitize(template.trim());
    }).join('');
}
...
function typeAhead(search) {
    ...
    input.on('input', function () {
        ...
        _axios2.default.get('/api/search/?q=' + value).then(function (result) {
    ...
    input.on('keyup', function (e) {
        ...
        var current = results.querySelector('.' + activeClass);
        var items = results.querySelectorAll('.search__result');
        var next = void 0;

        if (key === 40 && current) {
            // going down with something already selected
            next = current.nextElementSibling || items[0];
        } else if (key === 40) {
            // going down for the 1st time
            next = items[0];
        } else if (key === 38 && current) {
            // going up with something already selected
